/* Stock Timing Radar v8.2.4 Runtime Fix
...
(function stockcheckV824RuntimeFix(){
  ...
  function injectStyles(){
    if (document.getElementById('stockcheckV824RuntimeStyles')) return;
    const style = document.createElement('style');
    style.id = 'stockcheckV824RuntimeStyles';
    style.textContent = `
      .v824-ai-card{border:1px solid rgba(88,166,255,.42)!important;background:linear-gradient(135deg,rgba(31,111,235,.16),rgba(13,17,23,.92))!important;border-radius:16px!important;padding:16px 18px!important;margin-top:16px!important;color:#E6EDF3!important}
      .v824-ai-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:10px}.v824-ai-head strong{font-size:18px}.v824-ai-head span{font-size:12px;color:#8B949E;text-align:right}.v824-ai-card ul{margin:0;padding-left:20px;display:grid;gap:9px}.v824-ai-card li{line-height:1.45}.v824-ai-warn{color:#D29922;font-weight:800}
  ...
  function buildAiReasons(row, data){
    ...
    if (isSample) reasons.push(`<span class="v824-ai-warn">⚠️ Fundamental data source is sample/static placeholder; this is a preview, not live SEC data.</span>`);
    if (has(row.revenue)) {
  ...
  function findAiCards(){
    ...
    candidates.forEach(h => {
      ...
      if (h.closest('.v824-ai-card, .v823-ai-card, .stockcheck-ai-view-patched')) return;
      if (norm(h.textContent).toLowerCase() !== 'ai view') return;
      let el = h.parentElement;
      for (let i = 0; el && i < 8 && el !== document.body; i++, el = el.parentElement) {
        if (el.closest('.v824-ai-card, .v823-ai-card, .stockcheck-ai-view-patched')) return;
  ...
  function enhanceAiViews(){
    ...
    loadFundData().then(data => {
      cards.forEach(card => {
        ...
        if (card.dataset.v824AiTicker === ticker) return;
        ...
        card.classList.add('v824-ai-card', 'stockcheck-ai-view-patched');
        card.dataset.v824AiTicker = ticker;
        card.innerHTML = `<div class="v824-ai-head"><strong>AI view</strong><span>${source}</span></div><ul>${reasons.map(r => `<li>${r}</li>`).join('')}</ul>`;
      });
    });
  }
  ...
  function isYahooAnalysisBlock(el){
    if (!el || el.dataset?.v824RecScanned === '1') return false;
  ...
  function patchRecommendationCards(){
    ...
    if (!cards.length) return;
    cards.forEach(card => { card.dataset.v824RecScanned = '1'; });
    loadRecData().then(data => {
      cards.forEach(card => {
        if (card.dataset.v824RecPatched === '1') return;
        const ticker = getTickerFromContext(card);
        if (!ticker) return;
        card.dataset.v824RecPatched = '1';
  ...
  function init(){
    ...
    window.__stockcheckV823Refresh = function(){
      ...
      document.querySelectorAll('[data-v824-rec-patched], [data-v824-rec-scanned]').forEach(card => {
        card.dataset.v824RecPatched = '';
        card.dataset.v824RecScanned = '';
      });
      document.querySelectorAll('[data-v824-ai-ticker]').forEach(card => { card.dataset.v824AiTicker = ''; });
      run();
    };
  }
