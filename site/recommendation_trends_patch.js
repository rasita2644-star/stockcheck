/* v8.1.7 Finnhub Recommendation Trends Chart
...
(function recommendationTrendsPatch(){
  ...
  function getTicker(card){
    ...
    return detailTicker ? detailTicker.toUpperCase() : "";
  }
  ...
  function patchCard(card){
    ...
    loadData().then(data => {
      mount.innerHTML = renderSection(ticker, data || {});
    });
  }

  function scan(){
    document.querySelectorAll(".yahoo-analysis-card").forEach(patchCard);
  }
  ...
  function init(){
    ...
    window.__stockcheckRecommendationTrendsRefresh = function(){ payload = null; loading = null; document.querySelectorAll(".yahoo-analysis-card").forEach(card => { card.dataset.recTrendsPatched = ""; card.querySelectorAll(".recommendation-mount").forEach(x => x.remove()); }); scan(); };
  }
